...
# ------------------------------------------------------------
# 0) Read DB secrets (from Streamlit Cloud -> App settings -> Secrets)
# ------------------------------------------------------------
try:
...
sql_preview = (
    ...
    if option == "employees"
    else "SELECT * FROM v_benchmark_matching LIMIT 10"
)

try:
    with engine.connect() as con:
        df_preview = pd.read_sql(text(sql_preview), con)
...
with st.form("benchmark_form", clear_on_submit=False):
    ...
    role_purpose = st.text_area(
        ...
        placeholder="Contoh: Melakukan analisis dan visualisasi data untuk mendukung pengambilan keputusan bisnis."
    )
...
if submitted:
    ...
    # Build list for Postgres text[] column
    selected_ids = [emp_options[k] for k in selected_people]

    # 3.2 insert benchmark row -> get job_vacancy_id
    try:
        insert_sql = text("""
    ...
    # 3.3 run matching for this benchmark id
    try:
        sql_rank = text("""
            SELECT
              v.employee_id,
