...
SELECT
  ...
  tm.tgv_name,
  tm.tv_name,
  ROUND(tm.baseline_score, 2) AS baseline_score,
  ROUND(tm.user_score, 2)     AS user_score,
  ROUND(tm.tv_match_rate, 2)  AS tv_match_rate,
  ROUND(tg.tgv_match_rate, 2) AS tgv_match_rate,
  ROUND(fm.final_match_rate,2) AS final_match_rate
FROM tv_match tm
LEFT JOIN tgv_match tg   ON tg.employee_id = tm.employee_id AND tg.tgv_name = tm.tgv_name
...
ORDER BY fm.final_match_rate DESC NULLS LAST, e.employee_id, tm.tgv_name, tm.tv_name;
